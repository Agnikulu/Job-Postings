...
| `eval/eval_gold.jsonl` | **Authoritative, committed accuracy set** (2455 rows) — independently hand-labeled against `RUBRIC.md`. `tests/test_eval_regression.py` and `tests/test_eval_metrics_floor.py` run against this in CI on every PR/push touching `*.py`. |
